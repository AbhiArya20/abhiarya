...
export default function OneKo({ className, speed = 10, x = 16, y = 16, file = "/oneko.gif" }: OneKoProps) {
  ...
  useEffect(() => {
    ...
    function idle() {
      ...
      if (idealTime.current > 10 && Math.floor(Math.random() * 200) === 0 && idealAnimation.current == null) {
        ...
        if (position.current.x > window.innerWidth - 32) available.push("scratchWallE");
        if (position.current.y > window.innerHeight - 32) available.push("scratchWallS");

        idealAnimation.current = available[Math.floor(Math.random() * available.length)];
      }
    ...
    function onAnimationFrame(timestamp: number) {
      ...
      if (lastFrameTimestamp.current === undefined || timestamp - lastFrameTimestamp.current > 100) {

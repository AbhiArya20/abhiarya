...
export default function Badges({ name, icon, url, className }: Tag & { className?: string }) {
  ...
  return (
    <Link href={url} target="_blank" rel="noopener" onClick={() => track(`${name}_badge_clicked`)}>
...
function BadgeWithoutLink({ name, icon, className }: Omit<Tag, "url"> & { className?: string }) {
  if (icon) {
    ...
    return (
      <Badge
        variant={"outline"}
        className={cn("bg-accent/50 border-muted-foreground rounded-sm border-dashed [&>svg]:size-3", className)}
      >
